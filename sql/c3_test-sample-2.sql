...
-- initial select all --
SELECT * FROM simulationparticipation;

-- 1) find which simulations belong to a user, for example grace --
SELECT id
FROM simulationparticipation
...
INSERT INTO `user_schema`.`simulationparticipation`(`username`,`id`,`isOwner`) 
SELECT 'april', 2, 0
WHERE (SELECT COUNT(username) FROM simulationparticipation WHERE id = 2) < 5 
AND (SELECT max(isOwner) + 0 FROM simulationparticipation WHERE id = 2) < 2;
...
INSERT INTO `user_schema`.`simulationparticipation`(`username`,`id`,`isOwner`) 
SELECT 'jeff', 1, 0
WHERE (SELECT COUNT(username) FROM simulationparticipation WHERE id = 1) < 5 
AND (SELECT max(isOwner) + 0 FROM simulationparticipation WHERE id = 1) < 2;
